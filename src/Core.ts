...
class Mvvm {
  private static readonly ALREADY: string = " has already been registered with Mvvm";
  public static register(name: string, supportedTags: string[], elementDecoratorClass: any): void | RegistrationError {

    if (Mvvm.factories[name]) {
      let err = new RegistrationError("'" + name + "'" + this.ALREADY);
      throw err;
    }
  ...
  public static registerFilter(name: string, fn: Function): void | RegistrationError {
    if (Mvvm.filters[name]) {
      let err = new RegistrationError("'" + name + "' filter" + this.ALREADY);
      throw err;
    }
