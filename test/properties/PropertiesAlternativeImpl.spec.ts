...
describe("unit tests for PropertiesImpl", () => {
	...
	test("Instantiates and available", () => {
	...
	test.skip("Keys are as expected", () => {
	...
	// - Null key passed, error thrown
	test(`testing isDefined(values: any): boolean`, () => {
		const spec: MutableProperties = new PropertiesAlternativeImpl();
		const wkSpy: MutableProperties = jest.spyOn(spec, 'isDefined');
		const result: MutableProperties = spec.load(p3);
		expect(result).not.toBeNull();
		const someProp: string = "a.b.e";
		expect(spec.isDefined(someProp)).toBe(true);
		expect(wkSpy).toHaveBeenCalledWith(someProp);
	});
	...
	// - Null key passed, error thrown
	test(`testing isTruthy(key: string): boolean being called - expect false`, () => {
		const spec: MutableProperties = new PropertiesAlternativeImpl();
		const wkSpy: MutableProperties = jest.spyOn(spec, 'isTruthy');
		const someProp: string = "somePropKey";
		const result: boolean = spec.isTruthy(someProp);
		expect(result).toBe(false);
		expect(wkSpy).toHaveBeenCalledWith(someProp);
	});

	// Properties::isFalsy(key: string): boolean;
	// - For "falsy" value for value - returfn true
	// - For non-"falsy" value for value - return value
	// - Null key passed, error thrown
	test(`testing isFalsy(key: string): boolean being called - expect true`, () => {
		const spec: MutableProperties = new PropertiesAlternativeImpl();
		const wkSpy: MutableProperties = jest.spyOn(spec, 'isFalsy');
		const someProp: string = "somePropKey";
		const result: boolean = spec.isFalsy(someProp);
	...
	// - Null key passed, error thrown
	test(`testing isPinned(key: string): boolean being called - expect false`, () => {
		const spec: MutableProperties = new PropertiesAlternativeImpl();
		const wkSpy: MutableProperties = jest.spyOn(spec, 'isPinned');
		const someProp: string = "somePropKey";
		const result: boolean = spec.isPinned(someProp);
		expect(result).toBe(false);
		expect(wkSpy).toHaveBeenCalledWith(someProp);
	});
	...
	// - Null key passed, error thrown
	test(`testing getAsString(values: any): string`, () => {
		const spec: MutableProperties = new PropertiesAlternativeImpl();
		const wkSpy: MutableProperties = jest.spyOn(spec, 'getAsString');
		const result: MutableProperties = spec.load(p3);
		expect(result).not.toBeNull();
		const someProp: string = "a.b.e";
		expect(spec.getAsString(someProp)).toEqual("e3");
		expect(wkSpy).toHaveBeenCalledWith(someProp);
	});
	...
	test(`testing load(values: any): MutableProperties`, () => {
		const spec: MutableProperties = new PropertiesAlternativeImpl();
		const wkSpy: MutableProperties = jest.spyOn(spec, 'load');
		const result: MutableProperties = spec.load(p3);
		expect(result).not.toBeNull();
		expect(wkSpy).toHaveBeenCalledWith(p3);
		const someProp: string = "a.b.e";
		expect(spec.isTruthy(someProp)).toBe(true);
	});
